    ...
    net_speed_widget = wibox.widget {
        {
	        ...
	        string.format('<span color="%s" font="'..beautiful.icon_font..'">龍</span>', beautiful.clr.orange)
	    ),
        },
        {
            id = 'rx_speed',
        ...
        set_rx_text = function(self, new_rx_speed)
            ...
            self:get_children_by_id('rx_speed')[1]:set_markup('<span color="'..beautiful.clr.orange..'"font="'..beautiful.widget_font..'">'..tostring(new_rx_speed)..' </span>')
        end,
        set_tx_text = function(self, new_tx_speed)
            --self:get_children_by_id('tx_speed')[1]:set_text(tostring(new_tx_speed))
            self:get_children_by_id('tx_speed')[1]:set_markup('<span color="'..beautiful.clr.orange..'"font="'..beautiful.widget_font..'">  '..tostring(new_tx_speed)..'</span>')
        end
    }
