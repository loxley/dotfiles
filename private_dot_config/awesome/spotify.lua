...
local GET_SPOTIFY_STATUS_CMD = 'playerctl -p spotify status'
local GET_CURRENT_SONG_CMD = "playerctl -p spotify metadata --format 'album:{{album}} artist:{{artist}} title:{{title}}'"
...
local function worker(user_args)
    ...
    spotify_widget = wibox.widget {
        ...
        set_status = function(self, is_playing)
            if is_playing then
                self:get_children_by_id('icon')[1]:set_markup('<span color="'..beautiful.clr.blue..'"font="'..beautiful.icon_font..'"> ﱘ </span>')
            else
                self:get_children_by_id('icon')[1]:set_markup('<span color="'..beautiful.clr.blue..'"font="'..beautiful.icon_font..'"> ﱙ </span>')
            end
        ...
        set_text = function(self, artist, song)
            ...
            self:get_children_by_id('artistw')[1]:set_markup('<span color="'..beautiful.clr.blue..'"font="'..beautiful.widget_font..'">'..artist_to_display..' - </span>')
            local title_to_display = ellipsize(song, max_length)
            self:get_children_by_id('titlew')[1]:set_markup('<span color="'..beautiful.clr.blue..'"font="'..beautiful.widget_font..'">'..title_to_display..'</span>')
        end
    }

    local update_widget_icon = function(widget, stdout, stderr, _, _)
        stdout = string.gsub(stdout, "\n", "")
        widget:set_status(stdout == 'Playing' and true or false)
    end

    local update_widget_text = function(widget, stdout, stderr, _, _)
        if string.find(stderr, 'No player') ~= nil then
            widget:set_text('','','')
        ...
        local album, artist, title =
            string.match(escaped, 'album:(.*) artist:(.*) title:(.*)\n')
    ...
    spotify_widget:connect_signal("button::press", function(_, _, _, button)
        if (button == 1) then
            awful.spawn("playerctl -p spotify play-pause", false)      -- left click
        elseif (button == 4) then
            awful.spawn("playerctl -p spotify next", false)  -- scroll up
        elseif (button == 5) then
            awful.spawn("playerctl -p spotify previous", false)  -- scroll down
        end
